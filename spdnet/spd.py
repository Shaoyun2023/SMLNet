...
class SPDTransform(nn.Module):
    ...
    def __init__(self, input_size, output_size):
        ...
        # 普通投影矩阵（随机初始化，无正交约束）
        # self.weight = nn.Parameter(torch.DoubleTensor(input_size, output_size), requires_grad=True)
        # nn.init.kaiming_normal_(self.weight)  # 或其他初始化方式（如Xavier）
...
class SPDIncreaseDim(nn.Module):
    ...
    def __init__(self, input_size, output_size):
        ...
        self.register_buffer('eye', torch.eye(output_size, input_size,dtype=torch.double))
        add = np.asarray([0] * input_size + [1] * (output_size-input_size), dtype=np.double)
    ...
    def forward(ctx, input):
        ...
        for k, x in enumerate(input):
            try:
                ...
                u, s, v = torch.svd(x + 1e-3 * x.mean() * torch.rand(1, 1089))
            # u, s, v = torch.svd(x)
            s.log_()
    ...
    def backward(ctx, grad_output):
        ...
        if ctx.needs_input_grad[0]:
            ...
            for k, g in enumerate(grad_output):
                ...
                #     u, s, v = torch.svd(x)
                # except:  # torch.svd may have convergence issues for GPU and CPU.
                #     u, s, v = torch.svd(x + 1e-3 * x.mean() * torch.rand(1, 224))
                u, s, v = torch.svd(x)
    ...
    def forward(ctx, input, epsilon):
        ...
        for k, x in enumerate(input):
            try:
                ...
                u, s, v = torch.svd(x + 1e-3 * x.mean() * torch.rand(1, input.size(1)))
                # u, s, v = torch.svd(x + 1e-3 * x.mean() * torch.rand(1, 1089))
